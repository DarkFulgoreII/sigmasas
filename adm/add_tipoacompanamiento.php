...
<html lang="es">
	<head>
		<meta charset="UTF-8"/>
		<title>Crear tipoacompanamiento</title>
		<?
			include_javascript();
		?>
	</head>
		...
		<? if(isset($_SESSION["userName"])): ?>
			<? if(!isset($_REQUEST["action"])){ ?>
				<div class="well" >
					...
					<h4>Crear tipoacompanamiento</h4>
			...
			<? }else if(isset($_REQUEST["action"]) && $_REQUEST["action"] == "guardar") { ?>
				<div class="well">
					<h4>tipoacompanamiento</h4>
					<div class="alert alert-success fade in">
					    <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
					    tipoacompanamiento ha sido guardado <strong>exitosamente</strong>
					</div>
					<center><a href="<?=$_SERVER['PHP_SELF']?>" class="btn btn-default btn-sm">Volver</a></center>
				</div>
			<? }?> 
